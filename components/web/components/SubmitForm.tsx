...
import { submitSkillAction } from "@/app/(root)/submit/actions";
...
export function SubmitForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const parseResult = submitSkillSchema.safeParse({
      name,
      slug,
      category,
      description,
    ...
    try {
      setErrors({});
      const res = await submitSkillAction(parseResult.data);
      if (res.success) {
        toast.success(`Successfully registered skill file: ${compiledFilename}.md!`);
        router.push(`/explore?filter=${res.category}`);
        router.refresh();
      }
    } catch (err: any) {
    ...
    <div className="max-w-2xl mx-auto bg-canvas border border-hairline rounded-xl p-8 shadow-xs">
      <div className="mb-8 border-b border-hairline-soft pb-6">
        ...
        <h2 className="font-sans text-2xl font-bold tracking-cal-sans-sm text-ink">
          Submit a New Skill File
        </h2>
        <p className="text-sm text-body mt-2 leading-relaxed">
          Provide a title, category, and raw prompt markdown template. It will be added to the registry for immediate indexing.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Button
          type="submit"
          disabled={loading}
          className="w-full h-10 mt-4 font-semibold bg-primary hover:bg-primary-active text-primary-foreground transition-colors duration-200 cursor-pointer"
        >
          {loading ? (
            <>
